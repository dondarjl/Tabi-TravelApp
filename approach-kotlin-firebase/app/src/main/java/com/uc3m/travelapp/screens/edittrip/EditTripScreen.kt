package com.uc3m.travelapp.screens.edittrip
...
import com.uc3m.travelapp.screens.addtrip.SectionTitle
...
fun EditTripScreen(
    ...
    // Load existing trip data and preload placesTexts from saved itinerary (MA-04)
        ...
        FirebaseManager.firestore
            ...
            .addOnSuccessListener { snapshot ->
                ...
                if (snapshot.exists()) {
                    ...
                    flightCost = (snapshot.getDouble("flightCost") ?: 0.0)
                        .let { if (it == 0.0) "" else it.toInt().toString() }
                    accommodationCost = (snapshot.getDouble("accommodationCost")
                        ?: snapshot.getDouble("hotelCost") ?: 0.0)
                        .let { if (it == 0.0) "" else it.toInt().toString() }
                    foodCost = (snapshot.getDouble("foodCost") ?: 0.0)
                        .let { if (it == 0.0) "" else it.toInt().toString() }
                    transportCost = (snapshot.getDouble("transportCost") ?: 0.0)
                        .let { if (it == 0.0) "" else it.toInt().toString() }
                    activitiesCost = (snapshot.getDouble("activitiesCost") ?: 0.0)
                        .let { if (it == 0.0) "" else it.toInt().toString() }
                    otherCost = (snapshot.getDouble("otherCost") ?: 0.0)
                        .let { if (it == 0.0) "" else it.toInt().toString() }
                    selectedCategories = (snapshot.get("categories") as? List<*>)
                        ?.filterIsInstance<String>()?.toSet() ?: emptySet()
                    currentCoverUrl = snapshot.getString("coverImageUrl") ?: ""

                    val rawItinerary = snapshot.get("itinerary") as? List<*>
                    if (!rawItinerary.isNullOrEmpty()) {
                        itinerary = rawItinerary.mapNotNull { item ->
                            (item as? Map<*, *>)?.let { map ->
                                DayEntry(
                                    dayNumber = (map["dayNumber"] as? Long)?.toInt() ?: 0,
                        ...
                        // Preload placesTexts from saved places so they appear in the fields (MA-02)
                        placesTexts = itinerary.associate { day ->
                            day.dayNumber to day.places.joinToString(", ")
                        }
                    }
                }
            }
            .addOnFailureListener {
                isLoadingTrip = false
                errorMessage = it.message ?: context.getString(R.string.detail_error_loading)
            }
    }

    // Regenerate itinerary when dates change, preserving existing day data (MA-02)
    LaunchedEffect(startDate, endDate) {
        if (startDate.isNotBlank() && endDate.isNotBlank() && !isLoadingTrip) {
            try {
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                title = {
                    Text(text = stringResource(R.string.edit_trip_title), fontWeight = FontWeight.Bold)
                },
                navigationIcon = {
                    IconButton(onClick = onBackClick) {
                        Icon(
                            imageVector = Icons.AutoMirrored.Filled.ArrowBack,
                            contentDescription = stringResource(R.string.edit_trip_cd_back)
                        )
                    }
                },
                    ...
                    Button(
                        onClick = {
                            ...
                            val updatedData = mutableMapOf<String, Any>(
                                ...
                                "endDate" to endDate,
                                "flightCost" to flight,
                                "accommodationCost" to accommodation,
                                "foodCost" to food,
                                "transportCost" to transport,
                                "activitiesCost" to activities,
                                "otherCost" to other,
                                "totalBudget" to (flight + accommodation + food + transport + activities + other),
                                "categories" to selectedCategories.toList(),
                                "itinerary" to itinerary.map { day ->
                                    mapOf(
                                        "dayNumber" to day.dayNumber,
                                        "places" to day.places
                                    )
                                }
                            )

                            if (selectedImageUri != null) {
                                val currentUser = FirebaseManager.currentUser ?: return@Button
                                ...
                                storageRef.putFile(selectedImageUri!!)
                                    .addOnSuccessListener {
                                        storageRef.downloadUrl.addOnSuccessListener { downloadUrl ->
                                            updatedData["coverImageUrl"] = downloadUrl.toString()
                                            saveEditedTrip(tripId, updatedData, context,
                                                onSuccess = { isLoading = false; tripSaved = true },
                        ...
                        Text(stringResource(R.string.edit_trip_save_button))
                    }
                }
            }
...
private fun saveEditedTrip(
    ...
    FirebaseManager.firestore
        ...
        .addOnFailureListener { exception ->
            onError(exception.message ?: context.getString(R.string.edit_trip_error_saving))
        }
}