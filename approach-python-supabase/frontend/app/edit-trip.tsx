...
type DayEntry = { date: string; morning: string; afternoon: string; dbId?: string };
type CostItem = { category: string; amount: string; dbId?: string };
...
export default function EditTripScreen() {
  ...
  // Foto
  const [coverUri, setCoverUri] = useState<string | null>(null);      // URI local nueva
  const [existingCoverUrl, setExistingCoverUrl] = useState<string | null>(null); // URL ya guardada

  // Secciones
  const [openSections, setOpenSections] = useState<Record<string, boolean>>({
    itinerary: false, costs: false, companions: false,
  });
  ...
  async function loadTripData() {
    ...
    try {
      ...
      if (error || !trip) { router.back(); return; }

      setTitle(trip.title || '');
      setDestination(trip.destination || '');
      setStartDate(trip.start_date || '');
      setEndDate(trip.end_date || '');
      setCost(trip.total_cost ? String(trip.total_cost) : '');
      setTripTypes(trip.trip_type || []);
      setExistingCoverUrl(trip.cover_photo_url || null);

      // Itinerario
      const { data: daysData } = await supabase
        .from('trip_days').select('*').eq('trip_id', id).order('day_number', { ascending: true });

      if (daysData && daysData.length > 0) {
        setDays(daysData.map(d => ({
          date: d.day_date,
          morning: d.morning_description || '',
          afternoon: d.afternoon_description || '',
          dbId: d.id,
        })));
      ...
      const { data: costsData } = await supabase
        .from('cost_items').select('*').eq('trip_id', id);

      if (costsData && costsData.length > 0) {
        const merged = COST_CATEGORIES.map(cat => {
          const existing = costsData.find(c => c.category === cat);
          return {
            category: cat,
        ...
        // Categorías personalizadas que no estén en la lista por defecto
        const custom = costsData.filter(c => !COST_CATEGORIES.includes(c.category));
        const customMapped = custom.map(c => ({ category: c.category, amount: String(c.amount), dbId: c.id }));
        setCostItems([...merged, ...customMapped]);
        setOpenSections(prev => ({ ...prev, costs: true }));
      }

      // Compañeros
      const { data: compData } = await supabase
        .from('companions').select('user_id, users(id, username)').eq('trip_id', id);

      if (compData && compData.length > 0) {
        const companions = compData.map((c: any) => c.users).filter(Boolean);
        setSelectedCompanions(companions);
        setOpenSections(prev => ({ ...prev, companions: true }));
      }
  ...
  function toggleSection(key: string) {
    ...
    if (key === 'itinerary' && opening && days.length === 0) generateDays();
  }
  ...
  async function searchCompanions(text: string) {
    ...
    const { data } = await supabase
      ...
      .ilike('username', `%${text}%`).neq('id', me?.id ?? '').limit(5);
  ...
  async function uploadCoverPhoto(userId: string, tripId: string): Promise<string | null> {
    ...
    try {
      ...
      const { error } = await supabase.storage
        .from('trip-covers').upload(path, uint8Array, { contentType, upsert: true });
  ...
  async function handleSave() {
    ...
    try {
      ...
      if (coverUri) {
        const coverUrl = await uploadCoverPhoto(user.id, id);
        if (coverUrl) {
          await supabase.from('trips').update({ cover_photo_url: coverUrl }).eq('id', id);
        }
      }

      // 3. Actualizar itinerario — borrar y reinsertar
      if (openSections.itinerary) {
      ...
      // 4. Actualizar costes — borrar y reinsertar
      if (openSections.costs) {
        await supabase.from('cost_items').delete().eq('trip_id', id);
      ...
      // 5. Actualizar compañeros — borrar y reinsertar
      await supabase.from('companions').delete().eq('trip_id', id);
      if (openSections.companions && selectedCompanions.length > 0) {
        await supabase.from('companions').insert(
          selectedCompanions.map(c => ({ trip_id: id, user_id: c.id }))
        );
      }
  ...
  // Foto a mostrar: nueva seleccionada > URL existente > nada
  const displayPhoto = coverUri || existingCoverUrl;
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.form}>
        ...
        <TouchableOpacity style={styles.uploadZone} onPress={pickCoverPhoto} activeOpacity={0.7}>
          {displayPhoto ? (
            <>
              <Image source={{ uri: displayPhoto }} style={styles.coverPreview} resizeMode="cover" />
        ...
        <View style={styles.field}>
          ...
          <DateRangePicker
            ...
            onChange={(start, end) => {
              setStartDate(start);
              setEndDate(end);
              // Regenerar días si el itinerario está abierto
              if (openSections.itinerary) {
            ...
            {days.length === 0 ? (
              <View style={{ gap: 8 }}>
                <Text style={styles.sectionHint}>
                  Introduce las fechas para generar el itinerario automáticamente.
                </Text>
                {startDate && endDate && (
                  <TouchableOpacity style={styles.generateBtn} onPress={generateDays}>
                    <Text style={styles.generateBtnText}>Generar días</Text>
                  </TouchableOpacity>
                )}
              </View>
            ) : (
...
const styles = StyleSheet.create({
  ...
  saveBtn: { backgroundColor: '#D95F2B', borderRadius: 20, paddingHorizontal: 16, paddingVertical: 6 },
  ...
  changePhotoOverlay: { position: 'absolute', bottom: 0, left: 0, right: 0, backgroundColor: 'rgba(0,0,0,0.4)', paddingVertical: 6, alignItems: 'center' },
  ...
  generateBtn: { backgroundColor: '#D95F2B', borderRadius: 10, paddingVertical: 10, alignItems: 'center' },
